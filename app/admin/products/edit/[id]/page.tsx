...
export default function EditProductPage(props: { params: Promise<{ id: string }> }) {
  ...
  const removeFeature = (index: number) => {
    if (productData.features.length > 1) {
      const updatedFeatures = productData.features.filter((_: any, i: number) => i !== index)
      setProductData((prev: any) => ({
        ...prev,
        features: updatedFeatures,
      }))
  ...
  const calculateDiscount = () => {
    if (productData.price && productData.originalPrice) {
      ...
      if (originalPrice > price) {
        ...
        setProductData((prev: any) => ({ ...prev, discount }))
      }
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const filteredFeatures = productData.features.filter((feature: string) => feature.trim() !== "")
